...
var utils =
{
  ...
  moveOverRoomsEX: function (beginRoomName, endRoomName, creep) {
    ...
    // 映射为 RoomPosition 对象数组
    var exitPos = []

    if (!Memory.roomExits[exitKey]) {
      // 在内存中没有找到出口，需要计算
    ...
    var exitPos = Memory.roomExits[exitKey].map(posData => new RoomPosition(posData.x, posData.y, creep.room.name));

    if (exitPos.length > 1)
      var exit = creep.pos.findClosestByRange(exitPos);
    else
      var exit = exitPos
    ...
    if (creep.room.name === endRoomName) {
      ...
      creep.say("moveTo" + exit)
      console.log("creep.say(moveTo+exit)", "moveTo" + exit, creep.name)
      this.railwayMove(creep, exit, 0);
    }
  },
        ...
        if (roads.length > 0 && (roads[0].pos.x == exitPos.x || roads[0].pos.y == exitPos.y)) {
          ...
          for (var i = 0; i < roads.length; i++) {
            if ((roads[i].pos.x == exitPos.x || roads[i].pos.y == exitPos.y)) {
              ...
              if ((roadsnear.length + roadconstruction.length) == 2)
                Exits.push(exitPos);
            }
          }
        ...
        if (roads.length > 0 && (roads[0].pos.x == exitPos.x || roads[0].pos.y == exitPos.y)) {
          ...
          for (var i = 0; i < roads.length; i++) {
            if ((roads[i].pos.x == exitPos.x || roads[i].pos.y == exitPos.y)) {
              ...
              for (var i in roadconstruction)
                console.log("roadconstruction", i)
            }
          }
  ...
  railwayMove: function (creep, target, i) {
    if (!target) console.log("err", creep.name, target, i);
    if (!target) return;
    ...
    if (!creep.memory.lastPos) {
      ...
      if (creep.memory.lastPos.x === creep.pos.x && creep.memory.lastPos.y === creep.pos.y) {
        ...
        if (creep.memory.lastPos.stuckCount >= 10) {
          delete creep.memory.cachedPath;
          if (Memory.rooms[creep.room.name] && Memory.rooms[creep.room.name].roads[roadPosKey] && Memory.rooms[creep.room.name].roads[roadPosKey][targetPosKey])
  ...
  // 定义一个检测是否有通路的方法
  hasRoadOrConstructionSiteBetween: function (pos1, pos2) {
    // 创建一个新的PathFinder CostMatrix
    var startRoom = Game.rooms[pos1.roomName]
    var endRoom = Game.rooms[pos2.roomName]
    ...
    for (let y = 0; y < 50; y++) {
      for (let x = 0; x < 50; x++) {
        // 获取当前地块上的所有建筑
        let structures = Game.rooms[pos1.roomName].lookForAt(LOOK_STRUCTURES, x, y);
        let constructionSites = Game.rooms[pos1.roomName].lookForAt(LOOK_CONSTRUCTION_SITES, x, y);

        // 检查是否是道路或道路建筑工地
        if (!(structures.some(structure => structure.structureType === STRUCTURE_ROAD) ||
          constructionSites.some(site => site.structureType === STRUCTURE_ROAD))) {
          // 如果不是，将此地块的成本设置为255
    ...
    // 如果找到了路径且路径不为空，则返回true
    console.log("path.incomplete", path.incomplete, "path.path.length", path.path.length)
    return !path.incomplete && path.path.length > 0;
  },
